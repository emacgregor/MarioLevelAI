...
public class MyLevelGenerator{
	...
	private static final int CHROMOSOME_LENGTH = 200;
	private Random random = new Random();
	private static final int MUTATION_CHANCE = 50;
	private static final int POPULATION_SIZE = 100;
	private int currCycle = 0;
	private ArrayList<Double> bestFitnessOverTime = new ArrayList<Double>();
	...
	private MyDNA generateRandomIndividual ()
	{
		...
		individual.generateRandomly(CHROMOSOME_LENGTH);
	...
	private boolean terminate (ArrayList<MyDNA> population, int count)
	{
		...
		// YOUR CODE GOES BELOW HERE
		if (currCycle > 0){
			if (bestFitnessOverTime.get(currCycle - 1) == 1.0) {
				decision = true;
			}
		}
		if (currCycle > 100 && bestFitnessOverTime.get(currCycle - 1) >= 0.8) {
			if (bestFitnessOverTime.get(currCycle - 1) - 0.01 <= bestFitnessOverTime.get(currCycle - 100)) {
				decision = true;
			}
		}
		if (currCycle > 200) {
			if (bestFitnessOverTime.get(currCycle - 1) - 0.01 <= bestFitnessOverTime.get(currCycle - 200)) {
				decision = true;
			}
		}
		currCycle++;
	...
	private ArrayList<MyDNA> competeWithParents (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation, Hashtable parents)
	{
		...
		// YOUR CODE GOES BELOW HERE]
		//printPopulation(oldPopulation);
		//printPopulation(newPopulation);
		finalPopulation.addAll(oldPopulation);
		Collections.sort(newPopulation);
		for (int i = 0; i < newPopulation.size(); i++) {
			MyDNA newGuy = newPopulation.get(i);
			ArrayList<MyDNA> theParents = (ArrayList<MyDNA>) parents.get(newGuy);
			Collections.sort(theParents);
			for (int j = 0; j < theParents.size(); j++) {
				MyDNA parent = theParents.get(j);
				if (finalPopulation.contains(parent)) {
					if (newGuy.getFitness() > parent.getFitness()) {
						finalPopulation.add(newGuy);
						finalPopulation.remove(parent);
						break;
					}
				}
			}
	...
	private ArrayList<MyDNA> globalCompetition (ArrayList<MyDNA> oldPopulation, ArrayList<MyDNA> newPopulation)
	{
		...
		Collections.sort(finalPopulation, Collections.reverseOrder());
		finalPopulation = new ArrayList(finalPopulation.subList(0, this.getPopulationSize()));
